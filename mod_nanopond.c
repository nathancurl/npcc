...
static void *run(void *targ)
{
	...
	while (!exitNow) {
		...
		while ((pptr->energy)&&(!stop)) {
			...
			uint32_t mutation_occurred = (getRandomRollback(1) & 0xffffffff) < MUTATION_RATE;
			uint32_t tmp = getRandomRollback(mutation_occurred) * mutation_occurred;
			uint32_t is_inst = (tmp & 0x80) >> 7; // Shift right by 7 to get a 1 or 0
			uint32_t is_reg = !is_inst; // Use logical negation to ensure is_reg is either 1 or 0
			inst = (tmp & 0xf) * is_inst + inst * is_reg; // Update inst only if is_inst is 1
			reg = (tmp & 0xf) * is_reg + reg * is_inst; // Update reg only if is_reg is 1
